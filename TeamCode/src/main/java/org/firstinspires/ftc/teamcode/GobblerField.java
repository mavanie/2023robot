...
@TeleOp(name = "Gobbler Field Studio")
@Config
public class GobblerField extends LinearOpMode {
...
//        }
        ...
        double x = movingFastSideways ? 0 : -GobblerCommon.square(g1.left_stick_y) * (g1.a ? FAST_FORWARD_FACTOR : SLOW_FORWARD_FACTOR);
        double y = movingFastForward ? 0 : GobblerCommon.square(g1.left_stick_x) * (g1.a ? FAST_SIDEWAYS_FACTOR : SLOW_SIDEWAYS_FACTOR);
        ...
        vx = x * Math.cos(Math.toRadians(absoluteYaw))- y * Math.sin(Math.toRadians(absoluteYaw));
        vy = x * Math.sin(Math.toRadians(absoluteYaw)) + y * Math.cos(Math.toRadians(absoluteYaw));
